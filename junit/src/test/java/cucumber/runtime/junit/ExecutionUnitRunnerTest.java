...
public class ExecutionUnitRunnerTest {
    ...
    @Test
    public void shouldKeepWordWithUIDWithFilenameCompatibleNameOption() throws Exception {
        CucumberFeature cucumberFeature = TestFeatureBuilder.feature("featurePath", "" +
                "Feature: feature name\n" +
                "  Scenario: an example of a scenario outline can have so many or so long values that the generated Test xml file has a filename that is too long for linux so if there is a unique identified in the example like uid:312214093rjweeod183u12wjdo21 which is generated by hiptest then the filename will use only that word\n" +
                "    Then it works"
                );
        ...
        assertEquals("uid_312214093rjweeod183u12wjdo21", runner.getName());
        assertEquals("Then_it_works", runner.getDescription().getChildren().get(0).getMethodName());
    }
